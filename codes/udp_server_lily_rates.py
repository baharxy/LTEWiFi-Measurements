...
#get input options
try:
    options, remainder = getopt.getopt(sys.argv[1:], 'p:m:s:w:l:h', ['SERVER_PORT=',
                                                             'MODE=',
                                                             'PACKET_SIZE=',
    ...
    print (str(e))
    print("Usage: %s -p server port -m u/d mode -s packet_size in bytes -w wifi_rate -l lte_rate" % sys.argv[0])
    sys.exit(2)
...
if MODE=='uplink':
 ...
 os.chdir(directory) 
 file_name='udp_oneway_' + '%s' %(PACKET_SIZE) +'bytes_' +'%s' %(NR_OF_PACKETS) +'packets'+ '%s' %(PACKETS_PER_SEC_WIFI)+'sec'+ '%s' %(PACKETS_PER_SEC_LTE)+'sec'+datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S_%f')[ :-3] +'.csv'
 while True:
 ...
 if not os.path.exists(directory):
  os.makedirs(directory)
 os.chdir(directory)
 tcpdump_output='tcpdump_' + '%s' %(PACKET_SIZE) +'bytes_' +'%s' %(NR_OF_PACKETS) +'packets'+ '%s' %(PACKETS_PER_SEC_WIFI)+'sec' + '%s' %(PACKETS_PER_SEC_LTE)+'sec' + datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S_%f')[ :-3] + '.csv'

  
#sending SIDE
def udp_server_send(IP, PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC_WIFI, PACKETS_PER_SEC_LTE, offset, input_sock,interface_type):
...
if __name__ == "__main__":
 ...
 if MODE=='downlink':
  if success==1:
   ...
   wifi_sender_thread = threading.Thread(target=udp_server_send, args=(wifi_IP, wifi_PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC_WIFI, PACKETS_PER_SEC_LTE,offset,ServerSocket,'wifi'))
   #wifi_sender_thread.daemon=True
   wifi_sender_thread.start()
   LTE_sender_thread = threading.Thread(target=udp_server_send, args=(NAT_IP, NAT_PORT, PACKET_SIZE,NR_OF_PACKETS, PACKETS_PER_SEC_WIFI, PACKETS_PER_SEC_LTE,offset,ServerSocket,'lte'))
