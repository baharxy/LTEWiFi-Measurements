...
#get input options
try:
    options, remainder = getopt.getopt(sys.argv[1:], 'a:p:m:s:w:l:h', ['SERVER_IP','SERVER_PORT=',
                                                                 'MODE=',
                                                                 'PACKET_SIZE=',
                                                                 'PACKETS_PER_SEC_WIFI=',
                                                                 'PACKETS_PER_SEC_LTE=',
                                                                 'help'                                                             ])
except getopt.GetoptError as e:
    print (str(e))
    print("Usage: %s -a server ip -p server port -m u/d mode -s packet_size in bytes -w wifi packet rate -l lte packet rate " % sys.argv[0])
    sys.exit(2)
for opt, arg in options:
    ...
    if opt in ('-p', '--SERVER_PORT'):
        ...
        PACKET_SIZE =int(arg)
    elif opt in ('-w', '--PACKETS_PER_SEC_WIFI'):
        PACKETS_PER_SEC_WIFI=int(arg)
    elif opt in ('-l', '--PACKETS_PER_SEC_LTE'):
        PACKETS_PER_SEC_LTE=int(arg)    
    elif opt == '--help':
        print("Usage: %s -a server ip -p server port -m u/d mode -s packet_size in bytes -r_w packets per second wifi -r_l packets persecond lte" % sys.argv[0])
print 'SERVER_IP   :', SERVER_IP
print 'SERVER_PORT   :', SERVER_PORT
print 'MODE   :', MODE
print 'PACKET SIZE    :', PACKET_SIZE
print 'PACKETS_PER_SEC_WIFI    :', PACKETS_PER_SEC_WIFI
print 'PACKETS_PER_SEC_LTE    :', PACKETS_PER_SEC_LTE
...
RATE_WIFI= float(float(PACKET_SIZE) * float(PACKETS_PER_SEC_WIFI) *  8 / 1000000 )#data rate mbps
RATE_LTE= float(float(PACKET_SIZE) * float(PACKETS_PER_SEC_LTE) *  8 / 1000000 )#data rate mbps

print ('WiFis Transmission rate is:  %3f'  %(RATE_WIFI) + ' Mbit/s')
print ('LTEs Transmission rate is:  %3f'  %(RATE_LTE) + ' Mbit/s')
...
if not os.path.exists(directory):
    ...
    os.chdir(directory)
if MODE=='uplink' :
 file_name='udp_rtt_' + '%s' %(PACKET_SIZE) +'bytes_' +'%s' %(NR_OF_PACKETS) +'wifipackets'+ '%s' %(PACKETS_PER_SEC_WIFI)+'sec'+ 'ltepackets'+ '%s' %(PACKETS_PER_SEC_LTE)+'sec'+ datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S_%f')[ :-3] +'.csv'
 tcpdump_output='tcpdump_' + '%s' %(PACKET_SIZE) +'bytes_' +'%s' %(NR_OF_PACKETS) +'wifipackets'+ '%s' %(PACKETS_PER_SEC_WIFI)+'sec'+'ltepackets'+ '%s' %(PACKETS_PER_SEC_LTE)+'sec' + datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S_%f')[ :-3] + '.csv'
elif MODE=='downlink':
 file_name='udp_oneway_' + '%s' %(PACKET_SIZE) +'bytes_' +'%s' %(NR_OF_PACKETS) +'wifipackets'+ '%s' %(PACKETS_PER_SEC_WIFI)+'sec'+'ltepackets'+ '%s' %(PACKETS_PER_SEC_LTE)+'sec' + datetime.datetime.now().strftime('%Y-%m-%d_%H%M%S_%f')[ :-3] +'.csv'

#write transmission rate to the channel file
open(channels_file, "a").write('wifi Transmission rate: '+ str(RATE_WIFI)+'  Mbit/s' +'\n' + 'LTE Transmission rate: '+ str(RATE_LTE)+'  Mbit/s' +'\n')
...
if plat == 'OSX':
 ...
 ip_list = []
 for   interface  in netifaces.interfaces():
  if interface=='wlan1' or interface=='wwan0' or interface=='wlan0' or interface=='rmnet0' :
   for link in netifaces.ifaddresses(interface)[netifaces.AF_INET]:
    ip_list.append(link['addr'])

#CLIENT - RECEIVER
#Wifi Interface
UDP_RECEIVE_IP_wifi =ip_list[1] #'127.0.0.1' IP ADDRESS TO LISTEN FOR INCOMMING PACKETS (v4 or v6)
UDP_RECEIVE_PORT_wifi=60000 #IP PORT TO LISTEN FOR INCOMMING PACKETS
#4g interface
UDP_RECEIVE_IP_LTE = ip_list[0] #'127.0.0.1' IP ADDRESS TO LISTEN FOR INCOMMING PACKETS (v4 or v6)
...
#Local-Send
def udp_client_send(IP, PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC_WIFI,PACKETS_PER_SEC_LTE ,offset, input_sock,interface_type):
 #sleep  1 secs so tcpdump starts earlier
 time.sleep(1)
 inter_departure_time_wifi = 1./PACKETS_PER_SEC_WIFI
 inter_departure_time_lte = 1./PACKETS_PER_SEC_LTE
  ...
  if PACKET_SIZE >  50:
   ...
   for i in range (1,NR_OF_PACKETS+1):
   	 if interface_type =='wifi':
         time.sleep(inter_departure_time_wifi)
     elif interface_type=='lte':
     	 time.sleep(inter_departure_time_lte)
     if NR_OF_PACKETS==1:
...
if __name__ == "__main__":
 ...
 if MODE=='uplink':
    ...
    wifi_sender_thread = threading.Thread(target=udp_client_send, args=(SERVER_IP, SERVER_PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC_WIFI,offset,wifi_sock,'wifi'))
    #wifi_sender_thread.daemon=True
    wifi_sender_thread.start()
    
    LTE_sender_thread = threading.Thread(target=udp_client_send, args=(SERVER_IP, SERVER_PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC_LTE,offset,LTE_sock,'lte'))
  ...
  udp_client_send(SERVER_IP, SERVER_PORT, PACKET_SIZE, 1, PACKETS_PER_SEC_WIFI,offset,wifi_sock,'wifi')
  #send one packet by LTE interface to handshake with the NAT
  udp_client_send(SERVER_IP, SERVER_PORT, PACKET_SIZE, 1, PACKETS_PER_SEC_LTE,offset,LTE_sock,'lte')
