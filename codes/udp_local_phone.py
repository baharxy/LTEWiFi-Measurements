...
#get input options
try:
    options, remainder = getopt.getopt(sys.argv[1:], 'a:p:m:s:r:h', ['SERVER_IP','SERVER_PORT=',
                                                                 'MODE=',
                                                                 'PACKET_SIZE=',
                                                                 'PACKETS_PER_SEC=',
                                                                 'help'                                                             ])
    ...
    print (str(e))
    print("Usage: %s -a server ip -p server port -m u/d mode -s packet_size in bytes -r packet rate" % sys.argv[0])
    sys.exit(2)
for opt, arg in options:
    ...
    if opt in ('-p', '--SERVER_PORT'):
        ...
        print("Usage: %s -a server ip -p server port -m u/d mode -s packet_size in bytes -r packets per second" % sys.argv[0])
