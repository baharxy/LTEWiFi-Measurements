...
WIFI_IP_ADDR=$(echo $WIFI_IP_ADDR_MASK | cut -c 1-13)
LTE_IP_ADDR=$(echo $LTE_IP_ADDR_MASK | cut -c 1-14)    


WIFI_GATEWAY=$(ipcalc $WIFI_IP_ADDR_MASK | grep HostMin | cut -c 12-25)
LTE_GATEWAY=$(ipcalc $LTE_IP_ADDR_MASK | grep HostMin | cut -c 12-25)

WIFI_NETWORK=$(ipcalc $WIFI_IP_ADDR_MASK | grep Network | cut -c 12-28)
LTE_NETWORK=$(ipcalc $LTE_IP_ADDR_MASK | grep Network | cut -c 12-28)
