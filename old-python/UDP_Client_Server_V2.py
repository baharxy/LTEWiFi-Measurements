...
def udp_client_receive(UDP_RECEIVE_IP, UDP_RECEIVE_PORT,offset,input_sock, interface_type):
...
if __name__ == "__main__":
    ...
    wifi_sender_thread = threading.Thread(target=udp_client_send, args=(UDP_DEST_IP, UDP_DEST_PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC,offset,wifi_sock,'wifi'))
    #wifi_sender_thread.daemon=True
    wifi_sender_thread.start()
    
    LTE_sender_thread = threading.Thread(target=udp_client_send, args=(UDP_DEST_IP, UDP_DEST_PORT, PACKET_SIZE, NR_OF_PACKETS, PACKETS_PER_SEC,offset,LTE_sock,'lte'))
