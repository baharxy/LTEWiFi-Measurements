...
directory="/Users/bahar/Documents/tcpdumpData/python/csv/LTEcompare/"
folder_list=list.files(path=directory, pattern ="2016-03-03_*", all.files = FALSE)
#folder_list=c(folder_list,list.files(path=directory, pattern ="2016-03-02_145836*", all.files = FALSE))
#folder_list=c(folder_list,list.files(path=directory, pattern ="2016-03-02_150002*", all.files = FALSE))
...
mat <- matrix(1:n_plots,ncol=2,byrow=TRUE)
layout(mat)
...
for (folder in folder_list) {
  ...
  #otherwise do the measurements
    ...
    if (length(wifiRcvd$V4)!=0 ) {
     hist(wifiRcvd$V4,breaks=30, col='blue', xlab = "delay (sec)", ylab = "Frequency",main="",xlim = c(0,max(allRcvd$V4)))
    }
      if (length(lteRcvd$V4)!=0 ) { 
     hist(lteRcvd$V4,breaks=30, col='red',xlab = "delay (sec)", ylab = "Frequency",main="",xlim = c(min(allRcvd$V4),max(allRcvd$V4)))
      }
    }
    else{
      if (length(wifiRcvd$V4)!=0 ) {
      ...
      if (length(wifiRcvd$V4)!=0 ) {
        hist(lteRcvd$V4,breaks=30, add=T, col='red',main="", xlab="",ylab = "Frequency",xlim = c(min(allRcvd$V4),max(allRcvd$V4)))
      }
      else {
      if (length(lteRcvd$V4)!=0 ) {
      hist(lteRcvd$V4,breaks=30, col='red',main="", xlab="",ylab = "Frequency",xlim = c(min(allRcvd$V4),max(allRcvd$V4)))
      }
      }
    }
    #ggplot hist
