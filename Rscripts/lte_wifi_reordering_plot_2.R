...
folder_list=list.files(path=directory, pattern ="2016-03-12_182738_927_OSX_downlink", all.files = FALSE)
#folder_list=c(folder_list,list.files(path=directory, pattern ="2016-04-20_151351_102_LINUX_downlink", all.files = FALSE))
#folder_list=c(folder_list,list.files(path=directory, pattern ="2016-04-20_152621_929_LINUX_downlink", all.files = FALSE))
...
for (folder in folder_list) {
  ...
  p1 <- ggplot(allRcvd ) +     
    geom_histogram( aes(x=delay. , y=..count../sum(..count..), fill=interface, color=interface), binwidth=0.0005, position='identity', alpha=0.5 ) + 
    scale_fill_manual(values=c("navyblue", "rosybrown2" ))  + scale_colour_manual(values=c("blue", "red")) + xlab("delay (sec)" )+ ylab("density")
  ...
  ) +  xlab("delay (sec)" ) +  theme(legend.position="top") +
  ...
  #scatter arrivals
  #setEPS()
  p3<- ggplot(allRcvd,aes(x=arrivalTime,y=packet)) + geom_point(aes(colour=interface, fill = interface))+
    scale_fill_manual(values=c("white", "white"))  + scale_colour_manual(values=c("blue", "red"))+xlab("Arrival Time (s)")+ ylab("Packet number")
  ...
  ggsave(paste("/Users/bahar/Documents/tcpdumpData/python/RPlots/",folder,'/arrivals.eps',sep=""))
